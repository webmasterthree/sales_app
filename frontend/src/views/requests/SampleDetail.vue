    ...
    <template #extra="{ doc }">
      <div v-if="doc.items?.length" class="bg-surface rounded-2xl border border-rule p-4 space-y-2">
        ...
        <div v-for="row in doc.items" :key="row.name" class="flex justify-between text-sm">
          <span class="text-ink-2">{{ row.item_name || row.item_code }}</span>
          <span class="font-display">{{ row.qty }} {{ row.uom || "" }}</span>
        </div>
      </div>
      ...
      <CommentThread doctype="Sample Request" :docname="doc.name" />
    </template>
  </DetailView>
</template>
...
const config = {
  title: (doc) => doc?.customer_name || doc?.prospect_name || doc?.name || "Sample Request",
  method: "field_sales.api.requests.sample_request",
